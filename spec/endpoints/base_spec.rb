require 'spec_helper'

describe 'Instore::Api' do
  context 'with invalid token' do
    let(:access_token) { 'xxxxxxxxxxxxx' }
    subject { Instore::Api.new(access_token) }

    it 'should handle fetch', vcr: {cassette_name: 'categories/index_invalid_token'} do
      response = subject.categories.fetch
      response.status.should be_false
      response.data.should be_nil
    end

    it 'should handle find', vcr: {cassette_name: 'categories/show_invalid_token'} do
      response = subject.categories.find('xxxxxxxx')
      response.status.should be_false
      response.data.should be_nil
    end
  end

  context 'response' do
    subject { Instore::Api.new('9JkAmIetQcJg_ZJ-w-GEU9EphBg=') }
    
    it 'should throw exception for non existing fields', vcr: {cassette_name: 'categories/index'} do
      response = subject.categories.fetch
      expect { response.test }.to raise_error(Instore::UnsupportedMethod)
    end

    it 'should throw exception for non existing nested fields', vcr: {cassette_name: 'categories/index'} do
      response = subject.categories.fetch
      expect { response.data[0].test }.to raise_error(Instore::UnsupportedMethod)
    end
  end
end