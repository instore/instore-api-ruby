...
describe 'Instore::Api#items' do
  ...
  it 'should fetch items', vcr: {cassette_name: 'items/index'} do
    response = subject.items.fetch
    response.should be_instance_of(Instore::EndPoints::Mash)
  end

  it 'should fetch item', vcr: {cassette_name: 'items/show'} do
    response = subject.items.find('a259f474-1dba-e53f-d88f-5925cdeb8085')
    response.status.should be_true
  end
  
  it 'should create item', vcr: {cassette_name: 'items/create'} do
  ...
  it 'should update item', vcr: {cassette_name: 'items/update'} do
    response = subject.items.update('a259f474-1dba-e53f-d88f-5925cdeb8085', name: 'Test item 99!')
    response.status.should be_true
  end

  it 'should destroy item', vcr: {cassette_name: 'items/destroy'} do
