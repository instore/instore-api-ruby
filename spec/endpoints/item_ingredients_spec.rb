require 'spec_helper'

describe 'Instore::Api#item_ingredients' do
  let(:access_token) { '9JkAmIetQcJg_ZJ-w-GEU9EphBg=' }
  subject { Instore::Api.new(access_token) }

  it 'should fetch item_ingredients', vcr: {cassette_name: 'item_ingredients/index'} do
    response = subject.item_ingredients.fetch
    response.should be_instance_of(Instore::EndPoints::Mash)
  end

  it 'should fetch item_ingredient', vcr: {cassette_name: 'item_ingredients/show'} do
    response = subject.item_ingredients.find('bd2747f5-6ce5-f9be-78a7-1178ceb80cbb')
    response.status.should be_true
  end
  
  it 'should create item_ingredient', vcr: {cassette_name: 'item_ingredients/create'} do
    response = subject.item_ingredients.create(item_id: '57deb0d5-da5b-40c7-a5ed-3bfabc93f380', ingredient_id: '895cc2ea-19a7-a4ef-fb3a-b266f12dc9d0')
    response.status.should be_true
  end

  it 'should destroy item_ingredient', vcr: {cassette_name: 'item_ingredients/destroy'} do
    response = subject.item_ingredients.destroy('bd2747f5-6ce5-f9be-78a7-1178ceb80cbb')
    response.status.should be_true
  end
end